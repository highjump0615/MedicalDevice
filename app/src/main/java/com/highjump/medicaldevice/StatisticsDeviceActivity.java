...
public class StatisticsDeviceActivity extends HistoryActivity {

    private StatDeviceAdapter mAdapter;

    ArrayList<Device> maryData = new ArrayList<Device>();

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_stat_device);
        ...
        mAdapter = new StatDeviceAdapter(this, maryData);
    ...
    public void getData(final boolean bRefresh) {
        ...
        // 调用相应的API
        APIManager.getInstance().getDeviceList(
                ...
                new Callback() {
                    ...
                    public void onFailure(Call call, final IOException e) {
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                stopRefresh();
                                Toast.makeText(StatisticsDeviceActivity.this, e.getMessage(), Toast.LENGTH_SHORT).show();
                            }
                        });
                    }
                        ...
                        if (!response.isSuccessful()) {
                            ...
                            try {
                                ...
                                if (!resultObj.isSuccess()) {
                                    ...
                                    // 总数量
                                    mnTotalCount = jsonRes.getInt("deviceTotal");

                                    // 记录
                                    JSONArray aryJson = jsonRes.getJSONArray("devices");
                                    for (int i = 0; i < aryJson.length(); i++) {
                                        JSONObject jsonObj = aryJson.getJSONObject(i);
                                        Device newDevice = new Device(jsonObj);
                                        maryData.add(newDevice);
                                    }
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                if (!mstrApiErr.isEmpty()) {
                                    Toast.makeText(StatisticsDeviceActivity.this, mstrApiErr, Toast.LENGTH_SHORT).show();
                                    return;
                                }
